...
discover_handler(REQUEST* request, RESPONSE* response)
{
  char temp[100];
  int index = 0;
  index += sprintf(temp +index, "%s", "</heating>;n=\"heating\"");
...
RESOURCE(heating, METHOD_PUT | METHOD_GET, "heating");
void 
heating_handler (REQUEST* request, RESPONSE* response)
{
  if(rest_get_method_type(request)==METHOD_GET){
    char temp[100];
    rest_set_header_content_type(response, TEXT_PLAIN);
    sprintf(temp,"%d/100", heatingPercent);
    rest_set_response_payload(response, temp, strlen(temp));
  } else if(rest_get_method_type(request)==METHOD_PUT){
    char paramHeating[4];
    rest_get_post_variable(request, "heating", paramHeating, 4);
    uint8_t intValue = atoi(paramHeating);
    setHeater(intValue);
...
PROCESS(junkers_server, "Rest Server Example");
AUTOSTART_PROCESSES(&junkers_server);

PROCESS_THREAD(junkers_server, ev, data)
{
  heaterInit();
  PROCESS_BEGIN();
